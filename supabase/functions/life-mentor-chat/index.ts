...
// ── CORS ──────────────────────────────────────────────────────────────────────
// ALLOWED_ORIGIN env 变量支持逗号分隔多个域名，例如：
//   https://lifeos.yourdomain.com,https://www.lifeos.yourdomain.com
// 在 Supabase Dashboard → Project Settings → Edge Functions → Secrets 里配置
function getCorsHeaders(req: Request) {
  const origin = req.headers.get("origin") || "";
  const extraOrigins = (Deno.env.get("ALLOWED_ORIGIN") || "")
    .split(",")
    .map((o) => o.trim())
    .filter(Boolean);
  const isAllowed =
    origin === "" || // 本地直接调用（无 origin）
    origin.endsWith(".lovableproject.com") ||
    origin.endsWith(".lovable.app") ||
    extraOrigins.some((o) => origin === o);
  return {
    "Access-Control-Allow-Origin": isAllowed ? (origin || "*") : "https://note-lifeos.lovable.app",
